...
final class RefreshTokenRepository
{
     ...
     * @param string $rawToken  Le refresh token brut (sera hashé)
     ...
     * Utilisé lors d'un logout complet ou d'un reset password.
     *
    ...
    /**
     * Révoque tous les refresh tokens d'un utilisateur,
     * sauf celui de la session courante.
     * Utilisé lors d'un changement de mot de passe pour invalider
     * les autres appareils sans déconnecter l'appareil courant.
     *
     * @param int    $userId           L'ID de l'utilisateur
     * @param string $currentRawToken  Le token brut de la session à conserver
     * @return int Nombre de tokens révoqués
     */
    public function revokeAllExcept(int $userId, string $currentRawToken): int
    {
        $pdo = Db::pdo();
        ...
        $stmt->execute([
            'user_id'      => $userId,
            'current_hash' => hash('sha256', $currentRawToken),
        ]);
