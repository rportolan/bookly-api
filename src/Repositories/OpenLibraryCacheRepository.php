...
final class OpenLibraryCacheRepository
{
    ...
    public function upsert(string $cacheKey, string $query, string $responseJson, int $ttlSeconds): void
    {
        ...
        $ttlSeconds = max(60, min(60 * 60 * 24 * 30, $ttlSeconds));
        $expiresAt = (new \DateTimeImmutable('now'))
            ->modify('+' . $ttlSeconds . ' seconds')
            ->format('Y-m-d H:i:s');

        $stmt = $pdo->prepare("
            INSERT INTO open_library_cache (cache_key, query, response_json, expires_at)
            VALUES (:k, :q, :j, :exp)
    ...
    public function purgeExpired(int $limit = 500): int
    {
        $pdo = Db::pdo();
        $stmt = $pdo->prepare("
            DELETE FROM open_library_cache
