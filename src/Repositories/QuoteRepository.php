...
final class QuoteRepository
{
    ...
    public function create(int $userId, int $userBookId, string $content): array
    {
        $pdo = Db::pdo();

        // Insert only if the user owns the user_book
        $stmt = $pdo->prepare("
            INSERT INTO quotes (user_book_id, content)
            SELECT ub.id, :content
            FROM user_books ub
            WHERE ub.id = :user_book_id
              AND ub.user_id = :user_id
            LIMIT 1
        ...
        $stmt = $pdo->prepare("SELECT id, content, created_at FROM quotes WHERE id = :id LIMIT 1");
        $stmt->execute(['id' => $id]);
    ...
    public function update(int $userId, int $userBookId, int $quoteId, string $content): ?array
    {
        $pdo = Db::pdo();
        ...
        // 3) Return row
        $stmt2 = $pdo->prepare("SELECT id, content, created_at FROM quotes WHERE id = :id LIMIT 1");
